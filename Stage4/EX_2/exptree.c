...
struct tnode* createTree(int val, int type, char* c, int ntype, struct tnode *l, struct tnode *r)
{
    ...
    switch((ntype)){
    	case ps: 
    		 if(l->type==intptrtype || l->type==strptrtype){
    		 	if(r->type!=inttype && r->type!=l->type){
    		 		printf("Type Mismatch!\n");
    		 		exit(1);
    		 	}
    		 }else if(r->type==intptrtype || r->type==strptrtype){
    		 	if(l->type!=inttype && l->type!=r->type){
    		 		printf("Type Mismatch!\n");
    		 		exit(1);
    		 	}
    		 }else{
    		 	checktype(l,inttype,r,inttype);
    		 }
    		 
    		 break;
    	case ms: if(l->type==intptrtype || l->type==strptrtype){
    		 	if(r->type!=inttype && r->type!=l->type){
    		 		printf("Type Mismatch!\n");
    		 		exit(1);
    		 	}
    		 ...
    		 break;
    	case ml: if(l->type==intptrtype || l->type==strptrtype){
    		 	if(r->type!=inttype && r->type!=l->type){
    		 		printf("Type Mismatch!\n");
    		 		exit(1);
    		 	}
    		 ...
    		 break;
    	case dv: if(l->type==intptrtype || l->type==strptrtype){
    		 	if(r->type!=inttype && r->type!=l->type){
    		 		printf("Type Mismatch!\n");
    		 		exit(1);
    		 	}
    		 ...
    		 break;
    	case mod: if(l->type==intptrtype || l->type==strptrtype){
    		 	if(r->type!=inttype && r->type!=l->type){
    		 		printf("Type Mismatch!\n");
    		 		exit(1);
    		 	}
    		 ...
    		 break;
    	case asg: 
    		  if((l->type==intptrtype && r->left->Gentry->type!=inttype) || (l->type==strptrtype && r->left->Gentry->type!=strtype)){
    		  	printf("Type mismatch!\n");
    		  	exit(1);
    		  }
    	...
    	case arrval: t=lookup(temp->left->varname);
    		     ...
    		     break;
        case arr2dval: t=lookup(temp->left->varname);
    		     temp->Gentry=t;
    		     if(temp->right->left->type!=inttype && temp->right->right->type!=inttype){
    		     	printf("Array index expression is not of type int\n");
    		     	exit(1);
    		     }
    		     break;
        case arrasg: t=lookup(temp->left->varname);
    		     ...
    		     break;
        case arr2dasg: t=lookup(temp->left->varname);
    		     temp->Gentry=t;
    		     if(temp->right->left->type!=inttype && temp->right->right->type!=inttype){
    		     	printf("Array index expression is not of type int\n");
    		     	exit(1);
...
void assign_type(struct tnode* t,int type){
	...
	if(t->nodetype==id){
		...
		t->left->type=type;
		install(t->left->varname,type,t->right->val,0);
		t->left->Gentry=lookup(t->left->varname);
		return;
		
	}else if(t->nodetype==arr2ddecl){
		t->left->type=type;
		install(t->left->varname,type,t->right->left->val,t->right->right->val);
		t->left->Gentry=lookup(t->left->varname);
		return;
	}else if(t->nodetype==ptrdecl){
		if(type==inttype){
			t->left->type=intptrtype;
			install(t->left->varname,intptrtype,1,0);
			t->left->Gentry=lookup(t->left->varname);
		}else{
			t->left->type=strptrtype;
			install(t->left->varname,strptrtype,1,0);
			t->left->Gentry=lookup(t->left->varname);
		}
		return;
	}
...
void install(char* name,int type,int size,int size2){
	...
	s->binding=curr_SP;
	if(size2!=0){
		curr_SP=curr_SP+size*size2;
	}else{
		curr_SP=curr_SP+size;
	}
		...
		case ms : i=code_gen(t->left);
			  ...
			  return i;
                ...
                case rd : i=code_gen(t->left);
                	  ...
                	  }else if(t->left->nodetype==arrasg || t->left->nodetype==arr2dasg){
                ...
                case arrval: i=code_gen(t->right);
                	     ...
                	     return i;
                	     break;
                case arr2dval: i=code_gen(t->right->left);
                	       j=code_gen(t->right->right);
                	       fprintf(fp1,"MUL R%d,%d\n",i,t->left->Gentry->size2);
                	       fprintf(fp1,"ADD R%d,R%d\n",i,j);
                	       fprintf(fp1,"ADD R%d,%d\n",i,t->left->Gentry->binding);
                	       fprintf(fp1,"MOV R%d,[R%d]\n",i,i);
                	       free_reg();
                ...
                case arrasg:i=code_gen(t->right);
                	     ...
                	     return i;
                	     break;
                case arr2dasg:i=code_gen(t->right->left);
                	      j=code_gen(t->right->right);
                	      fprintf(fp1,"MUL R%d,%d\n",i,t->left->Gentry->size2);
                	      fprintf(fp1,"ADD R%d,R%d\n",i,j);
                	      fprintf(fp1,"ADD R%d,%d\n",i,t->left->Gentry->binding);
