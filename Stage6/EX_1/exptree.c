	...
	switch((t->nodetype)){
		...
		case asg: printf("hereeeee asg\n");
			...
			if(t->right->nodetype==allocnd){
				if(t->left->nodetype!=id && t->left->nodetype!=arrasg){
		...
		case id:printf("hereeeee id\n"); if(lookup(t->varname)==NULL && lsymbtable==NULL && LLookup(t->varname)==NULL){
	 	...
	 	case field:   struct Fieldlist* f2;
	 			if(t->left->nodetype==arrval){
	 		      	  if(strcmp(lookup(t->left->left->varname)->type->name,"INT")==0 || strcmp(lookup(t->left->left->varname)->type->name,"STR")==0){
	 		      	  	printf("Non-usr defined arr used\n");
	 		      	  	exit(1);
	 		      	  }
	 		      	  check_decl(type,t->left->right);
	 		      	  if(strcmp(t->left->right->type->name,"INT")!=0){
	 		      	  	printf("Arr expr is not of INT type\n");
	 		      	  	exit(1);
	 		      	  }
	 		      	  
	 		      	  f2=FLookup(lookup(t->left->left->varname)->type,t->right->varname);
	 		      }else{
	 		      	check_decl(type,t->left);
	 		      	f2=FLookup(t->left->type,t->right->varname);
	 		      }
